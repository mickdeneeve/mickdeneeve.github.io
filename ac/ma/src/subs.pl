...
newelt(NewList, OldList, N, P) :-
   append(OldPart1, OldPart2, OldList),
   append(OldPart1, [N|OldPart2], NewList),
   length(OldPart1, P), !. 

staborder(_,[], []).
staborder(Harm, [Stability|Tail], [Stability|Next]) :-
   member(Stability, Harm),
   staborder(Harm, Tail, Next), !.
staborder(Harm, [_|Tail], Next) :-
   staborder(Harm, Tail, Next).

unstabler(Note, [Note,Unstabler|_], Unstabler) :- !.
unstabler(Note, [_|Stabilities], Unstabler) :-
   unstabler(Note, Stabilities, Unstabler).
...
harm(Note, Stab, Harm) :-
   ( Stab = 0
    ...
    Harm is (Note - Stab) mod 7.
   %sub(Note, Stab, Harm).

sub(Note, Harm, Sub) :-
   Sub is (Note - Harm) mod 7.
